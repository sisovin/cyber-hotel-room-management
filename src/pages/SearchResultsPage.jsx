...
const generateMockRooms = (count = 100) => {
  const categories = ['Standard Pods', 'Deluxe Chambers', 'Cyber Suites', 'Zero-G Lofts', 'Presidential Matrix'];
  return Array.from({ length: count }, (_, i) => ({
    id: faker.string.uuid(),
    name: `${faker.helpers.arrayElement(['Nexus', 'Vortex', 'Oracle', 'Aura', 'Elysian'])} ${faker.helpers.arrayElement(['Pod', 'Chamber', 'Suite', 'Loft'])}`,
    ...
    image: `https://source.unsplash.com/random/400x400?futuristic,hotel,${i}`,
...
const SearchResultsPage = () => {
  ...
  useEffect(() => {
    setAllRooms(generateMockRooms());
    window.scrollTo(0, 0);
  }, []);

  const filteredAndSortedRooms = useMemo(() => {
    let searchFiltered = allRooms.filter(room =>
      query ? room.name.toLowerCase().includes(query.toLowerCase()) || room.category.toLowerCase().includes(query.toLowerCase()) : true
    );
    ...
    return filtered;
  }, [allRooms, query, filters, sortOption]);
  ...
  return (
    <div className="container mx-auto px-6 py-12 pt-32">
      ...
      <div className="text-center mb-12">
        <p className="text-teal mb-2">Search Results</p>
        <h1 className="text-4xl md:text-5xl font-extrabold text-white tracking-tighter">
          {query ? `Results for "${query}"` : 'All Accommodations'}
        </h1>
      </div>
        ...
        <main className="w-full lg:w-3/4">
          ...
          {filteredAndSortedRooms.length === 0 && (
            <div className="text-center py-24">
              ...
              <p className="text-gray-400 mt-2">Your search for "{query}" did not match any rooms. Try a different search or adjust your filters.</p>
            </div>
          )}
